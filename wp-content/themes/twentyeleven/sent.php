...
<script>
jQuery('.deletesentmessage').live('click',function() {
	var that = $(this).attr("id");
	jConfirm('Czy na pewno?', 'Potwierdzenie wyboru', function(r) {
		if(r == true)
		{
			var iddo = $("#do" + that).text();
			var iddata = $("#data" + that).text();
			var name = $("#name").text();
			$.ajax({
					url: "<?php echo get_bloginfo('template_url') ?>/pm/delete-message.php",	
					type: "POST",		
...
jQuery('.viewsentmessage').live('click', function() {
	var iddo = $("#do" + $(this).attr("id")).text();
	var iddata = $("#data" + $(this).attr("id")).text();
	var name = $("#name").text();
	$.ajax({
			url: "<?php echo get_bloginfo('template_url') ?>/pm/view-message.php",	
			type: "POST",		
			data: {typ : "wyslane", do : iddo, data : iddata, name : name},		
			cache: false,
			success: function (html) {	
