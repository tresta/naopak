...
function displayProducents($id) {
	$start = $id * 30 - 30;
	$connection = dbConnect();
	$result = mysql_query("SELECT user.ID AS ID, user.user_login AS userLogin, user.user_registered AS dateRegistered, dane.id, dane.imie AS name, dane.nazwisko AS surname, prod.nazwa AS nazwa FROM wpp_users AS user INNER JOIN s_dane_kontaktowe AS dane ON user.ID = dane.id_uzytkownik INNER JOIN s_producenci  AS prod ON user.ID = prod.id_uzytkownik WHERE user.user_login != 'admin' ORDER BY user.user_login LIMIT $start, 30")
	or die(mysql_error());
	...
	{		
		...
		while($row = mysql_fetch_array($result))
		{
			...
			echo '<td style="text-align:center" ><a href="' . get_bloginfo('url') . '/producent-details?id=' . $row['ID'] . '" class="viewuser" id="' . $i . '">Pokaż szczegóły</a></td>';
			echo '</tr>';
			echo '<tr>';	
			$i++;
		}
	...
	{
		echo '<tr>';
		echo '<td colspan="7">Brak producentów do wyświetlenia</td>';
		echo '</tr>';
	}
	echo '   
	</tbody>
