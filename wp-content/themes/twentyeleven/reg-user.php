...
/**
 ...
 Template Name: reg-user
 
 */
function add_scripts()
{
	echo '<script type="text/javascript" src="'.get_bloginfo('template_url').'/reg/jquery.js"></script>';
	echo '<script type="text/javascript" src="'.get_bloginfo('template_url').'/reg/validation.js"></script>';
...
<div class="rejestracjaForm">
    ...
    <div >
        <label for="email2">Powtórz adres email:</label><input id="email2" type="text" name="email2" /> 
        <br />
        <span id="email2Info"></span>
    </div>
    <div class="formclear"></div>
    <div class="leftForm">
        <label for="pass1">Hasło:</label><input id="pass1" type="password" name="pass1" />
        <br /> 
        <span id="pass1Info"></span>
    </div>
    <div>
        <label for="passl2">Powtórz hasło:</label><input id="pass2" type="password" name="pass2" /> 
        <br />
...
if (isset($_POST['submit'])) {
	...
	else
	{
		...
		wp_insert_user( array ('user_pass' => $user_pass, 'user_login' => $user_login,'user_nicename' => $user_nicename, 'user_email' => $user_email,'display_name' => $display_name,'role' => $role) ) ;
		
		//echo $email . $pass . '<br>' .  $token . $name1 . $surname . $comapny . $phone . $address . $city . $country . $data ;
		...
		$result = mysql_query( "SELECT `ID` FROM `wpp_users` WHERE `user_login` ='$user_login'")
		or die(mysql_error());  
		...
		$results = mysql_query( "INSERT INTO `s_dane_kontaktowe`(`id` ,`id_uzytkownik` ,`imie` ,`nazwisko` ,`adres` ,`miejscowosc` ,`telefon`, `firma` ,`kraj`) VALUES (NULL, '$user_id', '$name1', '$surname', '$address', '$city', '$phone', '$company', '$country');")
		or die(mysql_error());
		$resultt = mysql_query( "UPDATE `wpp_users` SET `token` = '$token' WHERE `ID` = '$user_id';")
		or die(mysql_error());    
		echo 'Użytkownik zarejestrowany';
		$to = $_POST['email'];
		
		$subject = 'NaOpak - potwierdzenie rejestracji';
		$message = 'Witamy,<br><br>';
		$message = 'Aby potwierdzić rejestrację na stronie NaOpak.pl proszę kliknąć poniższy link lub wkleić do pola adresu przeglądarki: ' . "<br><br>";
		$message .= '<a href="http://naopak.com.pl/reg-confirmation?u=';
		$message .= urlencode($email);
		$message .= '&amp;t=';
		$message .= urlencode($token) . '">Potwierdzenie rejestracji.</a>' . "<br><br>";
...
<div class="rejestracjaOpis">
<p>
•  gdy wypełnisz ten formularz, na podany adres email zostanie wysłany
email aktywacyjny, który musisz odebrać aby dokonczyć rejestrację
<br /><br />
•  jeśli w przyszlości zapomnisz hasła, skorzystaj z opcji przypomnienia
hasła dostępnej na stronie logowania •  jeśli chcesz aby Twoje zamówienia były wysyłane na adres slużbowy, podaj nazswę firmy. W przeciwnym razie poczta może nie dostarczyć przesyłki

</p>
<p>
•  jeśli chcesz aby Twoje zamówienia były wysyłane na adres slużbowy, podaj nazwę firmy. W przeciwnym razie poczta może nie dostarczyć przesyłki
<br /><br />
•  numer telefonu pomoże nam w kontakcie w razie jakichkolwiek wątpliwości dotyczących Twoich zamówień i nie będzie udostępniany
</p>
<p>
•  adres wysyłki podany podczas rejestracji stanie się domyślnym adresem, którego użyjemy do realizacji Twoich zamówień
<br /><br />
•  przy skladaniu zamówień będziesz mógł podać inny adres wysyłki
<br /><br />
•  jeśli podasz inny kraj niż Polska, automatycznie będziemy naliczać zagraniczny koszt wysyłki
</p>
</div>
 </div>   
